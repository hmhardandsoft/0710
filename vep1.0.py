...
# ================== 主界面 ==================
class TransientVEP:
    def __init__(self, online_mode=True, block_duration=60, required_refresh_rate=None):
        ...
        # 初始化采集线程 
        self.data_thread = DataAcquisitionThread(fs=2000, port='COM7', online_mode=self.online_mode)
    ...
    def init_ui(self):
        ...
        if self.required_refresh_rate is not None and self.refresh_rate != self.required_refresh_rate:
            print(
                f"[VEP] WARNING: expected {self.required_refresh_rate} Hz, "
        ...
        self.stim_frames = max(1, int(round(self.highlight_duration * self.refresh_rate)))
        self.total_frames = max(1, int(round(self.block_duration * self.refresh_rate)))
        self.soa_min_frames = max(self.stim_frames + 1, int(round(self.soa_min_sec * self.refresh_rate)))
        self.soa_max_frames = max(self.stim_frames + 1, int(round(self.soa_max_sec * self.refresh_rate)))
        expected_dt_ms = 1000.0 / self.refresh_rate
        print(
            f"[VEP] Display: {self.width}x{self.height} @ {self.refresh_rate} Hz "
            f"(expected frame={expected_dt_ms:.3f} ms)",
            flush=True,
        )
        print(
            f"[VEP] Timing: highlight={self.stim_frames} frames "
            f"({self.stim_frames / self.refresh_rate * 1000:.2f} ms), "
            f"SOA range={self.soa_min_frames}-{self.soa_max_frames} frames "
            f"({self.soa_min_frames / self.refresh_rate * 1000:.2f}-"
            f"{self.soa_max_frames / self.refresh_rate * 1000:.2f} ms), "
            f"block={self.total_frames} frames ({self.block_duration:.2f} s)",
            flush=True,
        )
    ...
    def _render_frame(self, highlight):
        glClear(GL_COLOR_BUFFER_BIT)
        glLoadIdentity()
        self._draw_square(1.0 if highlight else 0.0)
        glFlush()

    def _wait_for_space(self):
        print("[VEP] Press SPACE to start; ESC to quit.", flush=True)
    ...
    def _schedule_pulse(self):
        ...
        self.offset_frame = self.onset_frame + self.stim_frames

        soa_sec = random.uniform(self.soa_min_sec, self.soa_max_sec)
        self.current_soa_frames = max(self.stim_frames + 1, int(round(soa_sec * self.refresh_rate)))
        self.next_onset_frame = self.onset_frame + self.current_soa_frames
        self.trigger_sent_for_onset = False
    ...
    def close_application(self):
        ...
        if self.data_thread.is_alive():
            self.data_thread.stop(filename_prefix="TransientVEP_1min_long")
        try:
    ...
    def run(self):
        ...
        try:
            while not glfw.window_should_close(self.window):
                ...
                if self.frame_idx >= self.next_onset_frame:
                    self._schedule_pulse()

                highlight = self.onset_frame <= self.frame_idx < self.offset_frame
                if self.frame_idx == self.onset_frame and not self.trigger_sent_for_onset:
                    self.data_thread.send_trigger(1)
                    self.trigger_sent_for_onset = True

                self._render_frame(highlight)
                glfw.swap_buffers(self.window)
                glfw.poll_events()
                ...
                if self.frame_idx > 5 and dt > expected_dt * 1.5:
                    dropped_frames += 1
                    if dropped_frames <= self.debug_drop_print_limit:
                        print(
                            f"[VEP] Dropped-frame warning: frame={self.frame_idx}, "
                            f"dt={dt * 1000:.3f} ms, "
            ...
            measured_fps = self.frame_idx / max(stimulus_time, 1e-9)
            drop_rate = dropped_frames / max(self.frame_idx, 1) * 100
