...
frequency is fixed, each trial starts with one configured initial phase, and
its stimulus duration is randomized within a configured range.
...
class Stage1Config:
    ...
    stimulus_duration_min_sec: float = 3.0
    stimulus_duration_max_sec: float = 5.0
...
class Stage1PhaseTransferExperiment:
    ...
    def _validate_timing(self) -> None:
        ...
        if cfg.stimulus_duration_min_sec <= 0 or cfg.stimulus_duration_max_sec <= 0:
            raise ValueError("Stimulus duration bounds must be > 0.")
        if cfg.stimulus_duration_min_sec > cfg.stimulus_duration_max_sec:
            raise ValueError("stimulus_duration_min_sec must be <= stimulus_duration_max_sec.")
        ...
        print(
            f"[Stage1] Stimulus duration: random {cfg.stimulus_duration_min_sec:.3f}.."
            f"{cfg.stimulus_duration_max_sec:.3f} s per trial (frame-quantized); "
            f"ITI={cfg.iti_min_sec:.3f}..{cfg.iti_max_sec:.3f} s.",
            flush=True,
        )
        print(
    ...
    def _make_trial_schedule(self) -> list[dict[str, float | int]]:
        ...
        stim_min_frames = max(1, int(round(cfg.stimulus_duration_min_sec * self.refresh_rate)))
        stim_max_frames = max(stim_min_frames, int(round(cfg.stimulus_duration_max_sec * self.refresh_rate)))
        ...
        for block_idx in range(cfg.block_count):
            ...
            for trial_in_block, phase_idx in enumerate(labels):
                # Randomize stimulus duration directly in frames so the realized duration
                # matches what is actually displayed. randint is inclusive at both ends,
                # so exact 3.0 s and 5.0 s are possible when those durations correspond
                # to integer frame counts at the detected refresh rate.
                stim_frames = random.randint(stim_min_frames, stim_max_frames)
                stimulus_duration_sec = stim_frames / self.refresh_rate

                iti_sec = random.uniform(cfg.iti_min_sec, cfg.iti_max_sec)
                iti_frames = max(1, int(round(iti_sec * self.refresh_rate)))
                iti_duration_sec = iti_frames / self.refresh_rate
    ...
    def _metadata(
        ...
        return {
            ...
            # Backward-compatible scalar used by the current analysis code.
            # It is deliberately the minimum duration, so any accepted analysis window
            # is guaranteed to fit inside every trial. Per-trial realized durations are
            # stored in trial_schedule under stimulus_duration_sec.
            "stimulus_duration_sec": float(cfg.stimulus_duration_min_sec),
            "stimulus_duration_min_sec": float(cfg.stimulus_duration_min_sec),
            "stimulus_duration_max_sec": float(cfg.stimulus_duration_max_sec),
            "stimulus_duration_mode": np.array(["random_frame_uniform"], dtype=object),
...
def make_config_from_main_settings() -> Stage1Config:
    ...
    SQUARE_SCALE = 0.22
    STIMULUS_DURATION_MIN_SEC = 3.0
    STIMULUS_DURATION_MAX_SEC = 5.0
    ITI_MIN_SEC = 0.8
    ITI_MAX_SEC = 1.2
    ...
    return Stage1Config(
        ...
        stimulus_duration_min_sec=STIMULUS_DURATION_MIN_SEC,
        stimulus_duration_max_sec=STIMULUS_DURATION_MAX_SEC,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stimulus-duration-min-sec", type=float, default=3.0)
    parser.add_argument("--stimulus-duration-max-sec", type=float, default=5.0)
...
def make_config_from_args(args: argparse.Namespace) -> Stage1Config:
    return Stage1Config(
        ...
        stimulus_duration_min_sec=args.stimulus_duration_min_sec,
        stimulus_duration_max_sec=args.stimulus_duration_max_sec,
