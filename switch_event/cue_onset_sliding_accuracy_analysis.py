...
Dataset/
  sub1/
    VEP_Exp2_M2_EEG_20260714_201635.mat
    VEP_Exp2_M2_States_20260714_201635.mat
    VEP_Exp2_M4_EEG_....mat
    VEP_Exp2_M4_States_....mat
...
def process_task_blocks(
    ...
    _save_task_outputs(subject, task_m, raw_blocks, cue_events, all_results, save_dir)
    return all_results
...
def _save_task_outputs(
    ...
    out_npy = save_dir / f"CueOnset_{subject}_M{task_m}_wins{win_tag}.npy"
    np.save(str(out_npy), all_results)
    print(f"  Saved results: {out_npy}")

    stats_path = save_dir / f"CueOnset_{subject}_M{task_m}_wins{win_tag}_stats.txt"
    _save_stats_text(stats_path, subject, task_m, raw_blocks, cue_events, all_results)
    _plot_task_results(all_results, subject, task_m, save_dir)
...
def _plot_task_results(all_results: dict, subject: str, task_m: int, save_dir: Path):
    ...
    for win_len, res in sorted(all_results.items()):
        ...
        ax.set_title(f"{subject} M{task_m} Cue-Onset Accuracy (win={win_len:.3f}s)")
        ax.set_ylim([-5, 105])
        ax.grid(True, alpha=0.3)
        ax.legend(loc="lower right", fontsize=8)
        fig.tight_layout()
        fig_path = save_dir / f"CueOnset_{subject}_M{task_m}_win{int(win_len * 1000)}ms_overall.png"
        ...
        for ax in axes[len(methods):]:
            ax.axis("off")
        fig.suptitle(f"{subject} M{task_m} Per-Label Accuracy (win={win_len:.3f}s)", fontsize=14)
        fig.tight_layout()
        fig_path = save_dir / f"CueOnset_{subject}_M{task_m}_win{int(win_len * 1000)}ms_per_label.png"
        fig.savefig(str(fig_path), dpi=150)
        plt.close(fig)
        print(f"  Saved per-label figure: {fig_path}")


def plot_group_full_mean(group_results_by_m: Dict[int, Dict[str, dict]], save_root: Path):
    """Plot subject-level full-window means by M, one figure per method/window length."""
    if not group_results_by_m:
        return
    summary_dir = save_root / "GroupSummary"
    summary_dir.mkdir(parents=True, exist_ok=True)

    task_ms = sorted(group_results_by_m)
    first_m = task_ms[0]
    first_subject = next(iter(group_results_by_m[first_m]))
    win_lens = sorted(group_results_by_m[first_m][first_subject])
    methods = list(group_results_by_m[first_m][first_subject][win_lens[0]]["acc_curves"].keys())
    colors = _method_colors()
    ...
    for win_len in win_lens:
        ...
        for m_name in methods:
            ...
            for task_m in task_ms:
                subj_vals = []
                for subj_res in group_results_by_m.get(task_m, {}).values():
                    if win_len in subj_res:
                        subj_vals.append(subj_res[win_len]["full_mean_acc"][m_name])
                if subj_vals:
                    arr = np.asarray(subj_vals, dtype=float)
                    means.append(float(np.nanmean(arr)))
                    sems.append(float(np.nanstd(arr, ddof=1) / np.sqrt(arr.size)) if arr.size > 1 else 0.0)
                else:
                    means.append(np.nan)
                    sems.append(np.nan)
            ax.errorbar(
        ...
        ax.set_title(f"Group Full-Window Mean Accuracy (win={win_len:.3f}s)")
        ax.set_xticks(task_ms)
        ax.set_ylim([-5, 105])
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig_path = summary_dir / f"GroupFullMean_win{int(win_len * 1000)}ms.png"
        fig.savefig(str(fig_path), dpi=150)
        plt.close(fig)
...
def process_subject(subj_dir: Path, save_root: Path):
    ...
    if not pairs:
        print(f"  [SKIP] {subject}: no paired VEP_Exp2_M*_EEG/States_*.mat files.")
        return {}
    ...
    for task_m, blocks in sorted(raw_blocks_by_m.items()):
        out_dir = save_root / subject / f"M{task_m}"
        result = process_task_blocks(subject, task_m, blocks, out_dir)
        if result is not None:
            subject_results_by_m[task_m] = result
...
def main():
    ...
    group_results_by_m: Dict[int, Dict[str, dict]] = {}
    for subj_dir in subj_dirs:
        subj_results = process_subject(subj_dir, save_root)
        for task_m, result in subj_results.items():
            group_results_by_m.setdefault(task_m, {})[subj_dir.name] = result

    plot_group_full_mean(group_results_by_m, save_root)
